...
class GmailClient:
    ...
    def _parse_message(self, msg: dict) -> dict:
        ...
        text_body, html_body, attachments = self._extract_body_and_attachments(payload, msg["id"])
    ...
    def _extract_body_and_attachments(
        ...
        Returns:
            (text_body, html_body, attachments) 튜플
        """
        text_body = ""
        html_body = ""
        attachments = []
        ...
        if mime_type.startswith("multipart/"):
            for part in payload.get("parts", []):
                part_text, part_html, part_attachments = self._extract_body_and_attachments(part, message_id)
                if part_text:
                    text_body = part_text
                if part_html:
                    html_body = part_html
    ...
    def trash_thread(self, thread_id: str) -> dict:
        """스레드 휴지통으로 이동."""
        result = self.service.users().threads().trash(userId="me", id=thread_id).execute()
        return {
            "id": result["id"],
            "status": "trashed",
        }
    ...
    def get_label(self, label_id: str) -> dict:
        """라벨 상세 조회."""
        result = self.service.users().labels().get(userId="me", id=label_id).execute()
        return {
            "id": result["id"],
            "name": result["name"],
    ...
    def update_label(
        ...
        updated = (
            self.service.users().labels().update(userId="me", id=label_id, body=result).execute()
        )
    ...
    def batch_modify_labels(
        ...
        Returns:
            BatchResult 객체
        """
        result = self.batch_processor.batch_modify_labels(message_ids, add_labels, remove_labels)
...
def get_all_accounts(base_path: Optional[Path] = None) -> list[str]:
    ...
    return [f.stem for f in accounts_dir.glob("*.json") if f.stem not in ("credentials",)]
...
def get_client(
    ...
    if not accounts:
        raise ValueError("등록된 계정이 없습니다. setup_auth.py --account <이름> 실행 필요")
