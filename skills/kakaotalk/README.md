...
```bash
# 기본 (서명 포함)
uv run --project .claude/skills/kakaotalk python scripts/kakao_send.py "구봉" "밥 먹었어?"
# → "밥 먹었어?\n\nsent with claude code" 전송

# 서명 없이
uv run --project .claude/skills/kakaotalk python scripts/kakao_send.py "구봉" "밥 먹었어?" --no-signature

# 보내고 창 닫기
uv run --project .claude/skills/kakaotalk python scripts/kakao_send.py "구봉" "밥 먹었어?" --close
```
...
# 채팅방 목록
uv run --project .claude/skills/kakaotalk python scripts/kakao_read.py --list

# 채팅방 검색
uv run --project .claude/skills/kakaotalk python scripts/kakao_read.py --search "검색어"
```
